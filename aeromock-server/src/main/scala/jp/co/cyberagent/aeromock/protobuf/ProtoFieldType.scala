...
object ProtoFieldType {
  ...
  case object BYTES extends ProtoFieldType[ByteString] {
    ...
    override def toDefaultValue(value: String): Either[Throwable, ByteString] = trye(getByteString(value))
  ...
  case class ENUM(typeName: String, values: Map[String, Int]) extends ProtoFieldType[Int] {
    override val defaultValue: Int = if (values.isEmpty) 0 else values.head._2
    ...
    override def toDefaultValue(value: String): Either[Throwable, Int] = {
      values.get(value) match {
        case Some(index) => Right(index)
        case None => Left(new Throwable(s"${value} is not acceptable value as default value.")) // TODO
      }
    }
